...
class simulation():
    ...
    def barotrauma(self,discharge,K,ps_diameter,ps_length,v_head,fish_depths,h_D,h_2, beta_0, beta_1):
        """
        ...
        - fish_depths (array): an array of fish acclimation depths (m)
        ...
        fish_depths = fish_depths * 0.3048       # ft to m
        h_D = h_D * 0.3048                       # ft to m
        h_2 = h_2 * 0.3048                       # ft to m
        
        # calculate velocities
        # if flow is different at input/outflow, probably pass v_1 and v_2
        ...
        p_1 = barotrauma.calc_p_1(p_2, fish_depths, h_2, density, v_1, v_2, head_loss)
    ...
    def node_surv_rate(self,length,status,surv_fun,route,surv_dict,u_param_dict,unit_params):
        """
        ...
        if status == 0:
            ...
            if surv_fun == 'a priori':
                ...
                # output is mortality probability
                if self.pop['beta_0'] != 0 or self.pop['beta_1'] != 0: # fish is affected by barotrauma
                    
                    baro_prob = self.barotrauma(unit_params['Qopt'], # discharge - is this another place?
                                                unit_params['roughness'], # K - roughness value
                                                unit_params['D'], # penstock diameter
                                                ps_length, # penstock length
                                                unit_params['intake_vel'], # velocity head at turbine inlet
                                                fish_depths, # unifrom random between 1 and 30
                                                2, # submergence depth of the draft tube outlet
                                                2, # elevation head at the downstream point
                                                self.pop['beta_0'],
                                                self.pop['beta_1'])
                    
                    # we want survival probability, not mortality probability
                    baro_surv = 1 - baro_prob
                    # survival probability considering blade strike and barotrauma
                    prob = strike_prob * baro_surv
